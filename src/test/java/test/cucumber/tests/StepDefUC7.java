...
public class StepDefUC7 extends TestCase {
	...
	@When("^The Clerk logs into ATS and enters \"([^\"]*)\" and enters non-existing info \"([^\"]*)\" and term does not end and registration does not start$")
	public void the_Clerk_logs_into_ATS_and_enters_and_enters_non_existing_info_and_term_does_not_end_and_registration_does_not_start(String arg1, String arg2) throws Throwable {
		ats = new ATServer(Config.DEFAULT_PORT);
		Thread t = new Thread(ats);
		t.start();
		try {
		...
		String output = ats.output;
		if(state == 2) {
			if(output.contentEquals("The student does not exist!\n")) {
				success = true;
			}
		}
		ats = null;
